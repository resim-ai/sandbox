...
def register_experience(id, s3_path):
    experience = Experience.from_dict({
        "description": "Demo experience for cheese.",
        "location": s3_path,
        "name": f"Cheese experience {id}",
    })
...
for cheese in cheeses:
    ...
    experience_contents = {"cheese": cheese}
    with open(experience_path / 'cheese.json', 'w') as f:
        json.dump(experience_contents, f)
    destination = s3_prefix + experience_id + "/"

    s3_client = boto3.client('s3')

    print(f"Pushing experience with {cheese}")
    push_to_bucket(
        s3_client,
        staging_path,
        experience_path,
