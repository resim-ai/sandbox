...
def run_batch_metrics():
    ...
    try:
        # Read batch config
        with open(
            "/tmp/resim/inputs/batch_metrics_config.json", "r", encoding="utf-8"
        ...
        for test_id, metrics_proto in job_to_metrics.items():
            try:
                for metric in metrics_proto.metrics:
                    if metric.name == "False Positives" and isinstance(
                        metric, ScalarMetric
                    ):
                        total_fp += metric.value
                    elif metric.name == "True Positives" and isinstance(
                        metric, ScalarMetric
                    ):
                        total_tp += metric.value
                    elif metric.name == "False Negatives" and isinstance(
                        metric, ScalarMetric
                    ):
            ...
            try:
                ...
                for metric_data in metrics_proto.metrics_data:
                    if metric_data.name == "Detection Score Series" and isinstance(
                        metric_data, SeriesMetricsData
                    ):
                        print(
                            f"Size of Metrics Data for test: {test_id} =  {metric_data.series.shape}"
                        )
                        test_scores = metric_data.series.tolist()
                    elif metric_data.name == "True Positive Series" and isinstance(
                        metric_data, SeriesMetricsData
                    ):
                        print(
                            f"Size of True Positive Series for test: {test_id} =  {metric_data.series.shape}"
                        )
                        test_tp = metric_data.series.tolist()
                    elif metric_data.name == "False Positive Series" and isinstance(
                        metric_data, SeriesMetricsData
                    ):
                        print(
                            f"Size of False Positive Series for test: {test_id} =  {metric_data.series.shape}"
                        )
                ...
                if (
                    ...
                    and test_tp
                    and test_fp
                    and len(test_scores) == len(test_tp) == len(test_fp)
                ):
        ...
        if combined_data:
            ...
            precision_curve, recall_curve = compute_precision_recall_curve(
                list(all_scores), list(all_tp), list(all_fp), total_ground_truth
            )

            # Downsample the curves
            import numpy as np

            downsample_factor = 20
            original_length = len(precision_curve)
            if original_length > downsample_factor:
                # Create evenly spaced indices for downsampling
                indices = np.linspace(
                    0,
                    original_length - 1,
                    min(original_length // downsample_factor, original_length),
                    dtype=int,
                )
                precision_curve = [precision_curve[i] for i in indices]
                recall_curve = [recall_curve[i] for i in indices]
                print(
                    f"Downsampled precision-recall curve from {original_length} to {len(precision_curve)} points"
                )
            else:
                print(f"Curve too short to downsample (length: {original_length})")

            print(
                f"Precision-Recall curve calculated with {len(precision_curve)} points"
            )
            print(
                f"Final Precision: {precision_curve[-1]:.3f}, Final Recall: {recall_curve[-1]:.3f}"
            )

            # Print some key points from the curve
            print(
                f"Top 5 precision values: {[f'{p:.3f}' for p in precision_curve[:5]]}"
            )
