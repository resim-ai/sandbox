...
_set_default RESIM_SANDBOX_BUILD_TAG_PREFIX "load_test_"
_set_default RESIM_SANDBOX_METRICS_BUILD_TAG_PREFIX "load_test_metrics_"
_set_default RESIM_SANDBOX_BUILD_BRANCH "load_test"
...
resim builds create \
      ...
      --description "A ReSim load test build." \
      --image "${_FULL_TAG}" \
...
resim metrics-builds create \
      --url "${RESIM_API_URL}" \
      --auth-url "${RESIM_AUTH_URL}" \
      --name "A ReSim load test metrics build." \
      --image "${_FULL_TAG}" \
      --project "${RESIM_SANDBOX_PROJECT}" \
      --version "${RESIM_SANDBOX_BUILD_VERSION}"
